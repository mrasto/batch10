try {
	node {
		...
		stage('Publish Rreport') {
			...
			publishHTML([allowMissing: false, alwaysLinkToLastBuild: false, keepAll: false, reportDir: 'target/site', \ 
		...
		stage('Deploy Application') {
			...
			ansiblePlaybook credentialsId: 'ubuntu-ssh', disableHostKeyChecking: true, installation: 'ansible 2.11.1', \
            inventory: '/etc/ansible/hosts', playbook: "${env.playbook}"
		}
	...
	emailext attachLog: true, attachmentsPattern: "${env.JOB_NAME}#${env.BUILD_NUMBER}-${currentBuild.result}.log", \
    body: "Your ${jobstatus}", subject: "Build Result - ${env.JOB_NAME}#${env.BUILD_NUMBER} - ${currentBuild.result}", to: "${devmail}"
